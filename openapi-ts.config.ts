 ...
 *
 * Input mặc định là spec tĩnh repo `market` commit sẵn, nên generate KHÔNG cần bật server.
 * Muốn đọc trực tiếp từ BE đang chạy thì set `OPENAPI_INPUT=http://localhost:5000/openapi.json`.
 * Spec cũ hơn code BE thì chạy `npm run openapi:export` bên `market` trước.
 *
